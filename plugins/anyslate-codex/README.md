...
Create the local Codex env file and register the AnySlate MCP server in Codex:
...
- creates `~/.anyslate/codex.env` if missing
- writes `[mcp_servers.anyslate]` into `~/.codex/config.toml`
- stores no token value in the Codex config

Then edit the token file:
...
Restart Codex or open a new thread after running the installer so the
`anyslate` MCP server is loaded into the callable tool surface.
...
If Codex MCP registration is missing, the script tells you to rerun
`install-user-env.sh`.
